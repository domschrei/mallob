...
#    lines=$(sort $HOST_FILE_PATH|uniq|wc -l)
  ...
  time mpirun --mca btl_tcp_if_include eth0 --allow-run-as-root -np ${AWS_BATCH_JOB_NUM_NODES} \
  --hostfile combined_hostfile -mono=test.cnf -satsolver=llcgllcgllgcllgcllgcllgcll -cbdf=0.9 \
  -cfhl=300 -log=/dev/null -ihlbd=0 -iflbd=0 -islbd=0 -ihlbd=0 -fslbd=0 -fhlbd=0 \
  -smcl=0 -hmcl=0 -s=1 -sleep=1000 -T=1000 -t=$availablecores -v=4
}
