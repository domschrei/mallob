...
void merge(const Reduceable& other) override {
    ...
    for (size_t x = 0; x < data.size()+otherSet.data.size(); x++) {
        ...
        assert((newElem > 0 && newElem < 1.0) || log_return_false("%.3f is an invalid remainder to reduce!\n", newElem));
...
SortedDoubleSequence& deserialize(const std::vector<uint8_t>& packed) override {
    ...
    assert(packed.size() % sizeof(double) == 0 || log_return_false("%i not a multiple of %i!\n", packed.size(), sizeof(double)));
