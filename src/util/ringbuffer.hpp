...
public:
    ...
    bool produceInTwoChunks(const int* data1, size_t size1, const int* data2, size_t size2, bool addSeparationZero) {
        ssize_t offset = ringbuf_acquire(_ringbuf, _producer, size1 + size2 + (addSeparationZero ? 1 : 0));
        if (offset == -1) return false;
        
        memcpy(_data+sizeof(int)*offset, data1, sizeof(int)*size1);
        memcpy(_data+sizeof(int)*(offset+size1), data2, sizeof(int)*size2);
        if (addSeparationZero) {
            int zeroToAppend = 0;
            memcpy(_data+sizeof(int)*(offset+size1+size2), &zeroToAppend, sizeof(int));
        }
