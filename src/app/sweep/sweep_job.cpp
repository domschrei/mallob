...
std::shared_ptr<Kissat> SweepJob::createNewSweeper(int localId) {
	...
	if (_params.satProfilingLevel() >= 0) {
		...
		// LOGGER(_sweeplogger,V4_VVER, "SWEEP [%i](%i) Profiling Dir = %s \n", _my_rank, localId, setup.profilingBaseDir.c_str());
	...
	// LOGGER(_sweeplogger,V2_INFO, "SWEEP JOB [%i](%i) create kissat shweeper \n", _my_rank, localId);
	float t0 = Timer::elapsedSeconds();
	...
	if (_is_root) {
		...
		//update2: for integration in fullcmake we do want the cnf to be reported back.
	...
	//Start already with depth 3, and accordingly doubled sweepvars and clauses
...
void SweepJob::tryReportToMallob() { //needs to be called from the main thread
	...
	// if (_running_sweepers_count.load()>0) {
		// LOGGER(_sweeplogger,V2_INFO, " SWEEP : mainthread waiting with staged report %i to Mallob because still solvers running: %i \n", _staged_solved_status, _running_sweepers_count.load());
		// return;
	// }
...
bool SweepJob::checkCrossCommNeedsAdvancing(const std::string &from) {
	...
	if (_staged_solved_status!=-1 || _terminate_all) {
		if (_clause_comm->hasLocalClausesLeftToShare()) {
			...
			// LOGGER(_sweeplogger,V2_INFO, "SWEEP wrapup: _clause_comm->hasLocalClausesLeftToShare() == false \n");
		}
	}
	return false;
}
...
void SweepJob::appl_communicate(int sourceRank, int mpiTag, JobMessage& msg) {
	...
	// LOGGER(_sweeplogger,V2_INFO, "Shweep rank %i: received custom message from source %i, mpiTag %i, msg.tag %i \n", _my_rank, source, mpiTag, msg.tag);
...
void SweepJob::cbReportIteration(int localId) {
	...
	assert(sweeper);
	auto stats = sweeper->fetchSweepStats();
	//writing this often mangled the response time log line, too close together
	// LOGGER(_sweeplogger,			  V2_INFO, "SWEEP solver [%i](%i) reports about iteration %i in dedicated .sweep file \n", _my_rank, localId, stats.curr_iteration);
	...
	// LOGGER(_sweeplogger,V2_INFO, "TO_COMPLETION    %i		    \n", _params.sweepToCompletion());
...
void SweepJob::reportStealLatencies(KissatPtr sweeper) {
	...
	{
		// LOGGER(_sweeplogger,V2_INFO, "SWEEP solver lock enter %i\n", sweeper->getLocalId());
		...
		if (_stealinfos_per_solver.size() == _nThreads) {
			// LOGGER(_sweeplogger,V2_INFO, "SWEEP sorting steal information for reporting\n");
			...
			// LOGGER(_sweeplogger,V2_INFO, "SWEEP reporting steal information, sorted all infos\n");
			// LOGGER(_sweeplogger,V2_INFO, "SWEEP reporting steal information in dedicated file .stealsum\n");
		...
		// LOGGER(_sweeplogger,V2_INFO, "SWEEP solver lock exit  %i\n", sweeper->getLocalId());
	}
...
bool SweepJob::skip_MPI_forNow() {
	...
	if (getVolume()==1) {
		// LOGGER(_sweeplogger,V3_VERB, "SWEEP [%i] SKIP MPI steal, we are the only MPI rank\n", _my_rank);
		return true;
	}
...
void SweepJob::sendWorkstealsViaMPI() {
	...
	for (auto &request : _worksteal_requests) {
		if (request.to_send) {
			...
			// if (getVolume()==1) {
				// LOGGER(_sweeplogger,V4_VVER, "SWEEP SKIP [%i](%i) steal loop --> we are the only MPI rank, no global steal \n", _my_rank, senderLocalId);
				// continue;
			// }
...
void SweepJob::cbImportEq(int *elit1, int *elit2, int localId) {
	...
	if (idx < eqs.size()) {
		*elit1 = eqs[idx];
		*elit2 = eqs[idx+1];
		// LOGGER(_sweeplogger,V1_WARN, "SWEEP import e[%i,%i] \n", *elit1, *elit2);
...
bool SweepJob::canSolverExitStealing(KissatPtr sweeper) {
	...
	// if (request.is_active) {
		// if (shweep_get_end_iteration_signal(sweeper->solver)) {
			// LOGGER(_sweeplogger,V3_VERB, "Sweeper [%i](%i) would like to exit stealing (iteration ended), but must wait for pending MPI msg\n", _my_rank, localId);
			// usleep(2000);
		// }
		// if (_terminate_all.load(std::memory_order_relaxed)) {
			// LOGGER(_sweeplogger,V3_VERB, "Sweeper [%i](%i) would like to exit stealing (job terminated), but must wait for pending MPI msg\n", _my_rank, localId);
			// usleep(2000);
		// }
		// return false;
	// }
	...
	// if (shweep_get_end_job_signal(sweeper->solver)) {
		// LOGGER(_sweeplogger,V3_VERB, "Sweeper [%i](%i) exit mallob steal due to end_job flag \n", _my_rank, localId);
		// return true;
	// }
...
void SweepJob::solverGoStealing(KissatPtr sweeper) {
	...
	//To this end, deposit a steal request in a shared memory array, the main MPI thread will then pick it up and send an appropriate MPI message
...
void SweepJob::cbContributeToAllReduce() {
	...
	// LOGGER(_sweeplogger,V4_VVER, "SWEEP BCAST Callback to AllReduce\n");
	...
	for (auto &sweeper : _sweepers) {
		...
		if (stats.curr_iteration < expected_iteration_of_next_round) {
			//edgecase: This sweeper is still stuck in a previous iteration, maybe because its last Kitten Call takes extremely long, and it didn't yet reach the point where it resets it's work statistics
			//Thus we catch that case here and filter out "left-over" statistics form the previous iteration
			LOGGER(_sweeplogger,V2_INFO, "caught left-over work statistics: work %i steppedover %i \n", work_sweeps, work_stepovers);
			work_sweeps = 0;
			work_stepovers = 0;
			work_resweeps = 0;
		}
...
void SweepJob::advanceAllReduction() {
	...
	_red->advance();
	// LOGGER(_sweeplogger,V4_VVER, "SWEEP [%i] SHARE hasResult() %i \n", _my_rank, _red->hasResult());
	if (_red->hasResult()) {
		extractAllReductionResult();
	}
}
...
std::vector<int> SweepJob::aggregateEqUnitContributions(std::list<std::vector<int>> &contribs) {
	...
	for (const auto &contrib : contribs) {
		...
		for (int j=0; j < eq_size; j+=2) {
			...
			// LOGGER(_sweeplogger,V1_WARN, "SWEEP aggr  eq: e[%i,%i] \n", elit1, elit2);
	...
	for (uint64_t litpair : set_of_eqs) {
		...
		// LOGGER(_sweeplogger,V1_WARN, "SWEEP dedup eq: e[%i,%i], litpair %zu %ll \n", elit1, elit2, litpair);
		aggregated[j++] = elit1;
		aggregated[j++] = elit2;
	}
...
std::vector<int> SweepJob::stealWorkFromAnyLocalSolver(int asking_rank, int asking_sourceLocalId) { //Parameters are only used for verbose logging, don't influence function behaviour
	...
	// LOGGER(_sweeplogger,V1_WARN, "SWEEP [%i] ask from [%i](%i) to steal from any local solver \n",_my_rank, asking_rank, asking_sourceLocalId);
...
std::vector<int> SweepJob::stealWorkFromSpecificLocalSolver(int localId) {
	...
	// LOGGER(_sweeplogger,V1_WARN, "SWEEP [%i] stealattempt into (%i) \n",_my_rank, localId);
	if (_terminate_all.load(std::memory_order_relaxed)) //sweeping finished globally, nothing to steal anymore
		return {};
	if ( ! _sweepers[localId]) {
		// LOGGER(_sweeplogger,V3_VERB, "SWEEP STEAL stealing from [%i](%i), shweeper does not exist yet\n", _my_rank, localId);
		return {};
	}
	KissatPtr sweeper = _sweepers[localId];
	if ( ! sweeper->solver) {
		// LOGGER(_sweeplogger,V3_VERB, "SWEEP STEAL stealing from [%i](%i), shweeper->solver does not exist yet \n", _my_rank, localId);
		return {};
	}
	...
	// LOGGER(_sweeplogger,V1_WARN, "SWEEP [%i] stealattempt into (%i) max : %i \n",_my_rank, localId, max_steal_amount);

	if (max_steal_amount < MIN_STEAL_AMOUNT)
		return {};

	// LOGGER(_sweeplogger,V2_INFO, "ß %i max_steal_amount\n", max_steal_amount);
	...
	// LOGGER(_sweeplogger,V3_VERB, "[%i] stealing from (%i), expecting max %i  \n", _my_rank, localId, max_steal_amount);
...
void SweepJob::crossjob_rootReceiveClauses(std::vector<int>  &&clauses) {
	...
	auto clause = reader.getNextIncomingClause();
	{
		...
		while (clause.begin != nullptr) {
			...
			if (clause.size==1) {
				// LOGGER(_sweeplogger,V2_INFO, "  sconsume (only <=1): %s\n", clause.toStr().c_str());
...
void SweepJob::loadFormula(KissatPtr sweeper) {
	...
	// LOGGER(_sweeplogger,V2_INFO, "SWEEP Loading Formula, size %i \n", payload_size);
