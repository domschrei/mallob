...
namespace prod_cls {
    ...
    std::string toStr(const T& producedClause, int explicitLbdOrZero) {
        ...
        if constexpr (std::is_base_of<ProducedLargeClause, T>()) {
            std::string out = "len=" + std::to_string(producedClause.size - ClauseMetadata::numInts())
            ...
            for (size_t i = ClauseMetadata::numInts(); i < producedClause.size; i++)
...
struct ProducedClauseEquals {
    bool inline operator()(const T& a, const T& b) const {
        ...
        for (size_t i = ClauseMetadata::numInts(); i < prod_cls::size(a); i++) {
