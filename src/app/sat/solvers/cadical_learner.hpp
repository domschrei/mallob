...
public:
	...
	inline void learn(int lit) override {
		...
		bool eligible = true;
		if (_current_clause.size > ClauseMetadata::numBytes()+1) {
			assert(_current_clause.size >= ClauseMetadata::numBytes()+3); // glue value plus at least two literals
			...
			for (size_t i = ClauseMetadata::numBytes()+1; i > 0; i--) {
