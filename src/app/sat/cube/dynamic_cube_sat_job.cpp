...
DynamicCubeSatJob::DynamicCubeSatJob(Parameters& params, int commSize, int worldRank, int jobId)
    : Job(params, commSize, worldRank, jobId), _logger("<c-" + std::to_string(_world_rank) + std::string(toStr()) + ">", std::string(toStr())) {
    _dynamic_cube_comm = new DynamicCubeCommunicator(*this, _logger);
}
...
bool DynamicCubeSatJob::appl_initialize() {
    ...
    _lib = std::make_unique<DynamicCubeLib>(cube_setup);
