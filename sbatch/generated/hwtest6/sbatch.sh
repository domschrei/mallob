...
# HOME
globallogdir_base=logs/hwtest6-$SLURM_JOB_ID
# WORK
...
localtmpdir_base=/tmp/hwtest6-$SLURM_JOB_ID # fast local disk
...
ndone=$(echo sbatch/generated/hwtest6/.done.* | wc -w)
ntotal=$((8 - 0 + 1))
...
echo "I" >> sbatch/generated/hwtest6/.ticks
if [ $(cat sbatch/generated/hwtest6/.ticks|wc -l) -gt $ntotal ]; then exit; fi
...
for i in $(seq 0 8 | shuf) ; do
    ...
    # already done? -> skip
    if [ -d sbatch/generated/hwtest6/.done.$i ] ; then continue; fi
    # exit if you may be unable to finish this job in time
    if [ $(( $(date +%s) - $starttime + 300 + 30 )) -gt 360 ]; then 
		echo "Exit job because probably unable to finish in time"
		break; 
	fi
    # try to get a reservation for working on this job
    if [ -d sbatch/generated/hwtest6/.reserved.$i ] && ! [[ $(find sbatch/generated/hwtest6/.reserved.$i -newermt "8 minutes ago") ]]; then
        # reservation is old (>8 minutes) - reset it
        rmdir sbatch/generated/hwtest6/.reserved.$i
    fi
    # unable to get a reservation? -> skip
    if ! mkdir sbatch/generated/hwtest6/.reserved.$i 2> /dev/null ; then continue; fi
	...
	-v=4 -iff=0 -s2f=${globallogdir}/model -cm=0 \
    ...
    mkdir -p sbatch/generated/hwtest6/.done.$i
...
sbatch --begin=now`#+$((360+30))` sbatch/generated/hwtest6/sbatch.sh
