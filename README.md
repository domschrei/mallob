...
E.g., to run MallobSat with a single (MPI) process with twelve Kissat threads, execute `build/mallob -mono=path/to/problem.cnf -t=12 -satsolver=k`. Make sure to execute Mallob from it's home directory.

In case you observe `[ERROR] execl returned errno 2`, Mallob was not executed from it's home directory (see further: [FAQ:Execution](docs/faq.md#execution)).
