...
Note that such communication **may only be performed by each MPI process' main thread**. For example, a solver-thread that has a callback into Mallob should **not** itself send out MPI messages, but store the request in a local queue which is then picked up shortly afterwards by the MPI process' main thread.
