...
to run the script ```build_and_run_example.sh``` on a chosen target machine, here for example on ```blum```. We include compilation in the submitted job to have the code be compiled by the target machine itself, so that it can optimize the binary for its own specific CPU architecture. 
